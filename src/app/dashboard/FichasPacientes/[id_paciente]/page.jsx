...
export default function Paciente() {
    ...
    async function actualizarEstadoFicha(ficha, nuevoEstadoFicha) {
        setActualizandoEstadoId(ficha.id_ficha);

        try {
            const resFicha = await fetch(`${API}/ficha/editarFichaPaciente`, {
                method: "POST",
                headers: {
                    Accept: "application/json",
                    "Content-Type": "application/json"
            ...
            if (resultadoFicha.message !== true) {
                return toast.error("No fue posible actualizar el estado de asistencia");
            }

            // Sincronizar estado con la reserva asociada (silencioso, no bloquea el flujo).
            // Si no existe reserva vinculada el backend responde exitosamente con id_reserva: null.
            try {
                        ...
                        listaFichas.map((ficha) => (
                            <article key={ficha.id_ficha} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
                                ...
                                <div className="flex flex-col gap-3 sm:flex-row">
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => actualizarEstadoFicha(ficha, 4)}
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => actualizarEstadoFicha(ficha, 5)}
                                        disabled={actualizandoEstadoId === ficha.id_ficha}
